...
\subsubsection{lud, fft, srad, crc, nw, hmm, n-queens}
The LU-Decomposition {\tt lud}, Fast Fourier Transform {\tt fft}, Speckle Reducing Anisotropic Diffusion {\tt srad}, Cyclic Redundancy Check {\tt crc}, Needleman-Wunsch {\tt nw}, Baum-Welch Algorithm Hidden Markov Model {\tt hmm} and N-Queens {\tt n-queens} benchmarks did not require additional data sets.
...
The N-Queens application is a corner case where memory footprint scales poorly relative to the computational complexity of increasing the number of queens -- namely, valid placements of n queens on a chessboard of size n$\times$n where each queen cannot be attacked by another.
Thus, it is significantly compute bound and only only one size is provided.
...
\begin{table}[t]
	...
	\begin{threeparttable}
		...
		\begin{tabular}{l|l}
            ...
            \todo[inline]{verify the tiny and small size have n specified}
		\end{tabular}
		\begin{tablenotes}
