...
\subsubsection{lud, fft, srad, crc, nw}
...
\subsection{gem,nqueens,hmm}
The reasons for a range of problem sizes was to examine the memory heirachy accross a range of devices, unfortunately for a number of reasons the three applications mentioned here fail to meet this standard.
...
Unfortunately, the molecule used for the {\bf medium} and {\bf large} problem sizes contain uninitialized values only noticed on CPU architectures, as such, until tailoring of the datasets has occurred only the small result is used in the evaluation.

The N-Queens {\tt nqueens} benchmark is a backtrack/branch-and-bound code which finds valid placements of n queens on a chessboard of size n$\times$n where each queen cannot be attacked by another.
...
The Baum-Welch Algorithm Hidden Markov Model {\tt hmm} benchmark represents the Graphical Modelsdwarf and did not require additional data sets, however validation of the correctness of results has not occurred apart from over the {\bf tiny} problem size, as such, it is the only size examined in the evaluation.
...
\todo{Replace bfs if new results are available}
%\subsubsection{bfs}
%Breadth-First Search is a graph traversal method and computes the single-source shortest-path using fixed weights.
%Each graph is comprised of vertices and edges loaded from a file generated using the {\tt graphgen} tool provided in Rodinia\cite{che2009rodinia}.
%\todo[inline]{What is the distribution of edges generated by graphgen?}
