...
A subset of applications relevant to bioinformatics computation was closely examined in terms of their workload characteristics -- free from architecture and micro-architecture considerations -- and a discussion on how these characteristics impact the measured accelerator times is presented in Section\ \ref{sec:aiwc}.
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{figures/new-time-results/generate_main_2x4_bandwplot}
...
### Architecture-Independent Workload Characterization{#sec:aiwc}

To better understand the performance results for selected bioinformatics benchmarks, we analysed the codes using the Architecture Independent Workload Characterization (AIWC) tool [@johnston2018aiwc].
...
\begin{figure*}
    ...
    \subfloat[\texttt{nw}\label{fig:aiwc-nw}]{%
        \includegraphics[width=0.5\textwidth]{figures/aiwc-results/nw}
    }
    \subfloat[\texttt{swat}\label{fig:aiwc-swat}]{%
        \hspace*{-1.5cm}
        \includegraphics[width=0.58\textwidth]{figures/aiwc-results/sw-without-setZero}
    }
    \hfill
    \subfloat[\texttt{gem}\label{fig:aiwc-gem}]{%
        \includegraphics[width=0.5\textwidth]{figures/aiwc-results/gem}
    }
    \hspace*{-0.5cm}\subfloat[\texttt{kmeans}\label{fig:aiwc-kmeans}]{%
        \includegraphics[width=0.5\textwidth]{figures/aiwc-results/kmeans}
    }
    %\hfill
    %\vspace*{-1.5cm}\subfloat[\texttt{hmm}\label{fig:aiwc-hmm}]{%
    %    \hspace*{-1cm}\includegraphics[width=0.7\textwidth]{figures/aiwc-results/hmm}
    %}
...
\begin{figure*}
    \centering
    \includegraphics[width=1\textwidth]{figures/aiwc-results/hmm}
...
The bioinformatics subset of applications from the extended OpenDwarfs benchmark suite include computations used in sequence analysis, biophysics, gene expression/similarity and pattern identification.
...
The `kmeans` benchmark (Figure\ \ref{fig:aiwc-kmeans}) also has a high level of available parallelism and low branch and memory entropies; as expected, the measurements in Figure\ \ref{fig:time}a show that both modern GPUs and older HPC GPUs perform significantly better than CPUs for this benchmark.
...
\begin{figure*}
    ...
    \subfloat[][Kernel execution energy \label{fig:energy}]{%
        \includegraphics[width=0.5\textwidth]{figures/energy-results/energy_charts}
    }
    \subfloat[\texttt{Log of kernel execution energy}\label{fig:energy-log}]{%
        \includegraphics[width=0.5\textwidth]{figures/energy-results/energy_charts_log10}
    }
...
A major contribution of this work was to integrate LibSciBench into the benchmark suite, which adds a high precision timing library and support for statistical analysis and visualization.
...
The evaluation of performance bottle-necks for bioinformatics codes was presented using AIWC.
It is interesting to note that optimal accelerators are typically GPU based, given the high available thread parallelism and high barrier synchronization counts of many sequencing analysis applications.
However, the bioinformatics applications examined contain few kernels with higher branch and memory access entropies, interspersed with the GPU suited workloads, which suggests that CPUs are critical to achieve good performance on these systems.
...
Additional architectures such as FPGA, DSP modern AMD GPUs and Radeon Open Compute based APUs -- which further breaks down the walls between the CPU and GPU -- will be considered.
...
Analysis using AIWC helps understand how the structure of kernels contributes to the varying runtime characteristics between devices that are presented in this work, the examination of AIWC on the remaining kernels -- not associated to bioinformatics -- will be presented in the future.
...
Finally, the benefits of an architecture-independent evaluation are many.
AIWC may be used in the future to guide a developer to optimize kernels to specific accelerators, or tailor codes to generic accelerator types.
Metrics could be used to design better accelerators, or configure heterogeneous HPC nodes to have an optimal mix of accelerators for the primary characteristics of the most commonly executed scientific codes.
We envisage the use of AIWC for scheduling kernels to the optimal type of accelerator will grant better utilization of bioinformatics applications on future HPC systems -- which will be increasingly heterogeneous.
