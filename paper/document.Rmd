...
Examining the transition from tiny to large problem sizes (from left to right) in Figure\ \ref{fig:time2}b shows the performance gap between CPU and GPU architectures widening for `srad`, indicating that codes representative of structured grid dwarfs is well suited to GPUs.

In contrast, Figure\ \ref{fig:time2}c `nw` shows that the Intel CPUs and NVIDIA GPUs perform comparably for all problem sizes, whereas all AMD GPUs exhibit worse performance as size increases.
...
This reflects the higher latency and smaller size of the last-level cache on CPU devices relative to GPUs.
\todo{What is the memory size for the GPUs, and what is the difference in latency?}
It is expected if we had extended this study to an even larger problem size that would not fit on GPU global memory, much higher performance penalties would be experienced over GPU devices, since the PCI-E interconnect has a higher latency than a memory access to main memory from the CPU systems.
