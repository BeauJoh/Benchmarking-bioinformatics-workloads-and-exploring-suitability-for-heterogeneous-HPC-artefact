...
```{r eval=TRUE, dev='pdf', echo=FALSE, include=FALSE, cache=TRUE, draw_lud_diagonal_internal_all_kiviat, fig.cap = "\\label{fig:lud_diagonal_internal_kiviat} feature-space of the \\texttt{diagonal} and \\texttt{internal} kernels in the lud application on all problem sizes.", fig.width=8,fig.height=16}
...
The N-body Methods dwarf is represented by (`gem`) and the results are shown in Figure\ \ref{fig:time3}a, the Backtrack \& Branch and Bound dwarf is represented by the (`nqueens`) application in Figure\ \ref{fig:time3}b and (`hmm`) results in Figure\ \ref{fig:time3}c represent the Graphical Models dwarf.
...
In contrast, Figure\ \ref{fig:time2}c `swat` shows that the Intel CPUs and NVIDIA GPUs perform comparably over all problem sizes, whereas all AMD GPUs exhibit worse performance as size increases.
...
For multi-threaded or parallel workloads it collects metrics that indicate both instruction and thread-level parallelism.
Examples include coarse indications of parallelism by counting the number of work-items and barriers encountered, which in turn, can inform the developer on the amount of parallelism a given implementation of an algorithm achieves.
Additionally, distribution statistics around Instructions To Barrier (ITB), and Instructions per Thread (IPT) can be used to indicate both irregular workloads and load imbalance.

<!--
The **imbalance** shows the distribution of work between threads.
It is computed as $\frac{\mathrm{max instructions executed by a work-item }-\mathrm{min instructions executed by a work-item}}{\mathrm{total instruction count}}$.
-->
